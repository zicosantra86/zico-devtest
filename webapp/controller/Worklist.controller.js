sap.ui.define([
	...
	return BaseController.extend("au.com.bournedigital.developertest.controller.Worklist", {
		...
		onSearch: function (oEvent) {
			if (oEvent.getParameters().refreshButtonPressed) {
				...
				if (sQuery && sQuery.length > 0) {
					//aTableSearchState = [new Filter("Customer/CompanyName", FilterOperator.Contains, sQuery)];
		...
		showQuickViewEmp: function (oEvent) {
			var conText = oEvent.getSource().getBindingContext();
			var data = this.getView().getModel().getProperty(conText.sPath + "/Employee");
			var oEmployeeModel = new JSONModel();
			oEmployeeModel.setProperty("/pages",data);
		...
		openQuickView: function (oEvent) {
			...
			if (!this._pQuickView) {
				this._pQuickView = Fragment.load({
					id: oView.getId(),
					name: "au.com.bournedigital.developertest.fragment.QuickView",
					controller: this
